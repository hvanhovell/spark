...
import org.apache.spark.sql.catalyst.types.{PhysicalAtomicType, PhysicalBooleanType, PhysicalByteType, PhysicalDoubleType, PhysicalFloatType, PhysicalIntegerType, PhysicalLongType, PhysicalShortType, PhysicalStringType}
...
private[columnar] case object PassThrough extends CompressionScheme {
  ...
  override def encoder[T <: PhysicalAtomicType](columnType: NativeColumnType[T]): Encoder[T] = {
    new this.Encoder[T](columnType)
  }

  override def decoder[T <: PhysicalAtomicType](
      buffer: ByteBuffer, columnType: NativeColumnType[T]): Decoder[T] = {
    new this.Decoder(buffer, columnType)
  }

  class Encoder[T <: PhysicalAtomicType](columnType: NativeColumnType[T])
  ...
  class Decoder[T <: PhysicalAtomicType](buffer: ByteBuffer, columnType: NativeColumnType[T])
    ...
    override def decompress(columnVector: WritableColumnVector, capacity: Int): Unit = {
      columnType.physicalDataType match {
...
private[columnar] case object DictionaryEncoding extends CompressionScheme {
  ...
  override def decoder[T <: PhysicalAtomicType](buffer: ByteBuffer, columnType: NativeColumnType[T])
    : Decoder[T] = {
    new this.Decoder(buffer, columnType)
  }
...
private[columnar] case object BooleanBitSet extends CompressionScheme {
  ...
  override def encoder[T <: PhysicalAtomicType](columnType: NativeColumnType[T])
