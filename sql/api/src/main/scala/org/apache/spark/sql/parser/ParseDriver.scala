...
import org.apache.spark.sql.internal.{Origin, SqlApiConfigs}
import org.apache.spark.sql.parser.antlr.{SqlBaseLexer, SqlBaseParser, SqlBaseParserBaseListener}
import org.apache.spark.sql.types.{DataType, StructType}

/**
 * Base SQL parsing infrastructure.
 */
abstract class AbstractSqlParser[E <: DataTypeBuilder] extends DataTypeParser with Logging {

  protected def builder: E

  /** Creates/Resolves DataType for a given SQL string. */
  override def parseDataType(sqlText: String): DataType = parse(sqlText) { parser =>
    builder.visitSingleDataType(parser.singleDataType())
  }
  ...
  override def parseTableSchema(sqlText: String): StructType = parse(sqlText) { parser =>
    builder.visitSingleTableSchema(parser.singleTableSchema())
  }
  ...
  protected def parse[T](command: String)(toResult: SqlBaseParser => T): T = {
    ...
    parser.legacy_setops_precedence_enabled =
      SqlApiConfigs.setOpsPrecedenceEnforced
    parser.legacy_exponent_literal_as_decimal_enabled =
      SqlApiConfigs.exponentLiteralAsDecimalEnabled
    parser.SQL_standard_keyword_behavior =
      SqlApiConfigs.enforceReservedKeywords
    parser.double_quoted_identifiers =
      SqlApiConfigs.doubleQuotedIdentifiers
  ...
  protected def handleException(command: String, t: Throwable): Nothing = throw t
}
