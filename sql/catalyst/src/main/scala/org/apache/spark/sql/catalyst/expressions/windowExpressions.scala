 ...
 * Window Functions.
 */
sealed trait WindowSpecification extends Expression with Unevaluable {
  def partitionSpec: Seq[Expression]
  def orderSpec: Seq[SortOrder]
  def frameSpecification: Option[WindowFrame]
...
case class WindowSpecDefinition(
    ...
    frameSpecification: Option[WindowFrame])
  extends WindowSpecification {

  override def children: Seq[Expression] = partitionSpec ++ orderSpec ++ frameSpecification.toSeq

  override def checkInputDataTypes(): TypeCheckResult = {
    frameSpecification match {
      case None =>
        TypeCheckFailure(
          "Cannot use an unspecified window frame. This should have been " +
          "converted during analysis. Please file a bug report.")
      case Some(f) if f.frameType == RangeFrame && !f.isUnbounded && orderSpec.isEmpty =>
        TypeCheckFailure(
          "A range window frame cannot be used in an unordered window specification.")
      case Some(f) if f.frameType == RangeFrame && f.isValueBound && orderSpec.size > 1 =>
        TypeCheckFailure(
  ...
  override def sql: String = {
    def toSql(exprs: Seq[Expression], prefix: String): Seq[String] = {
      Seq(exprs).filter(_.nonEmpty).map(_.mkString(prefix, ", ", ""))
    }

    val elements =
      toSql(partitionSpec, "PARTITION BY ") ++
      toSql(orderSpec, "ORDER BY ") ++
      frameSpecification.map(_.sql).toSeq
    elements.mkString("(", " ", ")")
  }
 ...
 * under the name `name`.
 */
case class WindowSpecReference(name: String) extends WindowSpecification {
  override lazy val resolved = false
  override def partitionSpec: Seq[Expression] = Seq.empty
  override def orderSpec: Seq[SortOrder] = Seq.empty
  override def frameSpecification: Option[WindowFrame] = None
...
/**
 * A Window Frame defines the boundaries between which rows a processed to produce a
...
object WindowFrame {
  /**
   * Create a default window frame, given the ordering and the need for a frame.
   *
  ...
  def defaultWindowFrame(
      ...
      acceptWindowFrame: Boolean): WindowFrame = {
    if (hasOrderSpecification && acceptWindowFrame) {
      ...
      WindowFrame(RangeFrame, Unbounded, CurrentRow)
    } else {
...
case class WindowExpression(
    ...
    windowSpec: WindowSpecification) extends Expression with Unevaluable {
...
abstract class OffsetWindowFunction
  ...
  override lazy val frame: WindowFrame = {
    ...
    WindowFrame(RowFrame, boundary, boundary)
  }
...
abstract class AggregateWindowFunction extends DeclarativeAggregate with WindowFunction {
  self: Product =>
  override val frame: WindowFrame = WindowFrame(RowFrame, Unbounded, CurrentRow)
...
case class CumeDist() extends RowNumberLike with SizeBasedWindowFunction {
  ...
  override val frame = WindowFrame(RangeFrame, Unbounded, CurrentRow)
