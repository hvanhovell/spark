 ...
 * The only prerequisite for that is that the test class must extend [[BaseSparkFunSuite]].
 *
 ...
 * }
 */
abstract class BaseSparkFunSuite
  extends AnyFunSuite
...
// scalastyle:on
  ...
  protected def withTempDir(f: File => Unit): Unit = {
    val dir = CommonUtils.createTempDir()
    try f(dir) finally {
      CommonUtils.deleteRecursively(dir)
    }
  }
  ...
  protected def withSecretFile(contents: String = "test-secret")(f: File => Unit): Unit = {
    val secretDir = CommonUtils.createTempDir("temp-secrets")
    ...
    try f(secretFile) finally {
      CommonUtils.deleteRecursively(secretDir)
    }
  }
