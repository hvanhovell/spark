 ...
 * the string.
 */
class StringConcat(val maxLength: Int = CommonUtils.MAX_ROUNDED_ARRAY_LENGTH) {
  ...
  def append(s: String): Unit = {
    if (s != null) {
      ...
      length = Math.min(length.toLong + sLen, CommonUtils.MAX_ROUNDED_ARRAY_LENGTH).toInt
    }
  }
